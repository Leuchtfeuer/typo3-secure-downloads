...
Content-Length/Content-Range on a partial (206) response, which always describe the actual byte range read from disk
and are not influenced by this event. The name of the event is `Leuchtfeuer\SecureDownloads\Resource\Event\BeforeReadDeliverEvent`.
